...
const BlogList = () => {
        ...
        const filteredEvents = blogs.filter((item) => {
            ...
            if (menu === "official") {
                // Official events are those from residential colleges or university
                matchesCategory = item.eventCategory === 'residential_college' || item.eventCategory === 'university';
            } else {
                // For future/live/past, show only user-created events
                matchesCategory = item.status === menu && item.eventCategory === 'user';
            }
